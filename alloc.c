...
/* This will always point to the start of the memory pool. */
static unsigned char *start;
/* This will always point to the middle of the memory pool. */
...
/* 'toptr' will point to the 'to' portion of the memory pool for allocation. */
static unsigned char *toptr;
/* 'fromptr' will point to the 'from' portion and facilitate to-from copying. */
...
void reset_visited();
void copy_visited(const char *name, Reference ref);
...
void init_alloc(int memory_size) {
    ...
    /* The following pointers are initialized to function as described above. */
    start = mem;
    half = mem + MEMORY_SIZE / 2;
    toptr = mem;
    fromptr = half;
...
bool is_pool_address(void *addr) {
    return ((unsigned char *) addr >= start &&
            (unsigned char *) addr < start + MEMORY_SIZE);
}

/*! Returns true if the pool has the requested amount of space available. */
bool has_space_available(int requested) {
    /* Allocation memory divided in 2 for our stop and copy implementation. */
    return (toptr + requested <= mem + MEMORY_SIZE / 2);
}
...
Value * alloc(ValueType type, int data_size) {
    ...
    if (has_space_available(requested)) {
        ...
        /* Initialize the new Value in the bytes beginning at toptr. */
        new_value = (Value *) toptr;
...
void memdump() {
    ...
    fprintf(stdout, "Free  0x%08x; size %lu\n", (int) (toptr - mem),
        MEMORY_SIZE / 2 - (toptr - mem));
}

//// GARBAGE COLLECTOR ////

/*
 * This function receives a reference to a global variable and uses it to mark
 * all objects reachable from this reference as visited. Additionally, it is
 * responsible for copying visited objects to the fromspace.
 */
void copy_visited(const char *name, Reference ref) {
    (void) (name);
    if (ref == NULL_REF) {
    ...
    Value *curr_value = deref(ref);
    /* Checking if the object is marked avoids cycles in reachability. */
    if (curr_value->visited == 1) {
        return;
    }
    /* Mark as reached. */
    curr_value->visited = 1;

    int data_size = curr_value->data_size;
    int value_size = sizeof(Value) + data_size;
    /*
     * Copy the object from the to region to the from.  memcpy() does not worry
     * about overlapping source and destination, so if overlap occurs, data can
     * be corrupted as it is copied byte by byte, potentially overwriting data
     * bytes that should  be copied later. memmove() avoids such corruption from
     * overlap, but as a result runs slower since it must ensure overlap is
     * handled correctly. By virtue of our to and from pools, our stop and copy
     * implementation has no risk of overlap when copying, so the faster memcpy
     * is the correct choice.
     */
    memcpy(fromptr, curr_value, value_size);
    /* Update the object address in the reference table. */
    ref_table[ref] = (Value *) fromptr;
    /* Set fromptr for the next object copied to the from space. */
    fromptr += value_size;
    ...
    switch (curr_value->type) {
        ...
        case VAL_LIST_NODE: {
            /*
             * Both the list_node value and next fields must be recursively
             * called to  account for objects reachable from them.
             */
            ListValue *lv = (ListValue *) curr_value;
            copy_visited(name, lv->list_node.value);
            copy_visited(name, lv->list_node.next);
            break;
        }
        ...
        case VAL_DICT_NODE: {
            DictValue *dv = (DictValue *) curr_value;
            /*
             * The dict_node value, key, and next fields must be recursively
             * called to account for objects reachable from them.
             */
            copy_visited(name, dv->dict_node.value);
            copy_visited(name, dv->dict_node.key);
            copy_visited(name, dv->dict_node.next);
            break;
        }
        default:
            return;
...
/*
 * This function is responsible for unmarking copied objects and setting the
 * reference of any object determined to be garbage as a NULL_REF.
 */
void reset_visited() {
    Reference i;
    int new_numref = 0;
    /* Iterate through ref_table. */
    for (i = 0; i < num_refs; i++) {
        if (deref(i) == NULL) {
            continue;
        }
        Value *curr_value =  deref(i);
        /* If a value was not reached, its reference should reflect garbage. */
        if (curr_value->visited == 0) {
            curr_value->ref = NULL_REF;
            ref_table[i] = NULL;
        }
        else {
            /* Otherwise the visited field is reset to 0. */
            curr_value->visited = 0;
            /*
             * Updates the size of the ref_table based on highest reference of
             * non-garbage object.
             */
            new_numref = i;
        }
    }
    num_refs = new_numref + 1;
}
/*
 * This function uses the copy_visited() and reset_visited() functions defined
 * above to follow a stop and copy garbage collection implementation.
 */
int collect_garbage(void) {
    unsigned char *old_toptr = toptr;
    int reclaimed;

    printf("Collecting garbage.\n");
    /* Call the copying function on each global variable reference. */
    foreach_global(copy_visited);
    /*
     * After collection, the to and from regions flip. toptr always points to
     * fromptr so it is in the correct position for allocation.
     */
    if (fromptr > toptr) {
        toptr = fromptr;
        fromptr = start;
    ...
    /* Since the regions flip, old_toptr is modified to calculate reclaimed. */
    if (old_toptr < toptr) {
        old_toptr += MEMORY_SIZE / 2;
    }
    else {
        old_toptr -= MEMORY_SIZE / 2;
    }
    reclaimed = (int) (old_toptr - toptr);
    /* Reset the copied objects as unvisited and update the ref_table. */
    reset_visited();
...
void close_alloc(void) {
    /* Ensure mem points back to start of memory so pool is freed. */
    mem = start;
    free(mem);
    mem = NULL;
}
